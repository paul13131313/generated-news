...
## 完了済み（Phase 3: 位置情報パーソナライズ）

- [x] ブラウザ Geolocation API で位置情報取得
- [x] 許可拒否時のフォールバックUI（郵便番号入力モーダル + zipcloud API + Nominatim正引き）
- [x] 購読者プロフィールに location フィールド追加（lat, lng, area_name, prefecture, postal_code, source）
- [x] 逆ジオコーディング（Workers側 Nominatim + KVキャッシュ7日）
- [x] 天気コーナーを位置情報連動に変更
- [x] ご近所ニュースを位置情報連動に変更
- [ ] 催事を位置情報連動に変更（現在は全国共通のRSSソース）
- [x] 毎回のアクセス時に位置情報を更新（1km以上変化時のみ）
- [x] 設定画面に「位置情報を手動で変更」オプション（郵便番号モーダル + 現在地再取得）
