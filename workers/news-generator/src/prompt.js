...
- weatherFashion: items（服装アイテム配列）とcomment（一言天気コメント）を生成する。itemsは3〜5個の服装アイテム名のみ。ブランド名は絶対に含めない。長い説明文・形容詞・接続詞は一切不要。「コート」「ニット」「デニム」のような単語のみ。commentは「洗濯日和」「折りたたみ傘を」「重ね着がおすすめ」のような15字以内の一言。天気データは別途注入される
