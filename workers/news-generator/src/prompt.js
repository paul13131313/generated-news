...
- localNews: 恵比寿エリアのローカル情報3〜4件。新店オープン・閉店、地域イベント、再開発・工事情報、行列店や季節の風景など。住民目線の温かみのあるトーンで、各記事はtitle（見出し）とbody（本文2〜3文）。areaは"恵比寿"固定
- weatherFashion: 今日の東京の天気に合わせた服装提案。weather（天気）、tempHigh/tempLow（最高/最低気温）、rain（降水確率）は現実的な値を推測で生成。items（服装アイテム配列）は3〜5個の服装アイテム名のみ。ブランド名は絶対に含めない。長い説明文・形容詞・接続詞は一切不要。「コート」「ニット」「デニム」のような単語のみ
